...
![Version](https://img.shields.io/github/v/release/wonjun-lab/nfd2nfc?label=release)
...
파일이나 폴더(여러 개 동시 선택도 가능)에서 **우클릭 → 빠른 동작 → NFC로 이름 정리**.
폴더를 고르면 그 안쪽까지 한 번에 정리하고, 끝나면 알림이 뜹니다.
...
| 🖱️ Finder only (no terminal) | Download `nfd2nfc-quick-action.zip` from [Releases](https://github.com/wonjun-lab/nfd2nfc/releases/latest), unzip, and double-click `NFC로 이름 정리.workflow` |
